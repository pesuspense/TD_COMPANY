...
If answer = "ok" Then
	...
	If			field = "state"								Then
		...
		'If value = "d" Then
		'	fileName	= getExecQueryReValue("Select fileNameNew From page_file Where idx = '"& target &"'")
		'	filePath1 = server.mapPath(CFG_FOLDER_META&"") &"\"&				fileName
		'	filePath2	= server.mapPath(CFG_FOLDER_META&"") &"\delete\"& fileName
		'	If fileName <> "" Then
		...
		sql = ""
		sql = sql & "Update page_file Set "& field &" = '"& value &"' Where idx = '"& target &"'																														"							&vbCrlf	
	...
	ElseIf 	field = "step"								Then
		...
		For i = 1 To UBound(arr) +1
			If Trim(arr(i-1)) <> "" Then
				sql = sql & "update page_file Set step = "& i &" Where idx ="& arr(i-1)																																												&vbCrlf
			End If 
		Next
