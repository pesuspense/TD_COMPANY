...
sql						= getReadFile(server.mappath(CFG_FOLDER_META) &"\"& CFG_SES_NUM  &".txt")
setTarget			=	getClearstr(request("target"))
setType				=	getClearstr(request("type"))
setDivision		=	getClearstr(request("division"))

Call file_delete(server.mappath(CFG_FOLDER_META) &"\"& CFG_SES_NUM  &".txt")
'-----------------------------------------------------------------------------------------------------
'Process
'-----------------------------------------------------------------------------------------------------
If answer = "ok" Then
	...
	If Trim(sql) <> "\x" Then
		...
		sql = Replace(sql,"set_step"					,"(Select count(*) + 1  From page_file Where pageNum = '"& setTarget &"' And type = '"& setType &"')")
		Call execQuery(sql)
	End If
