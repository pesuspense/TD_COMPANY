...
If answer = "ok" Then
	...
	sql	= sql & "*																																						" &vbCrlf
	sql	= sql & "From contents_file																														" &vbCrlf
	sql	= sql & "Where state is Null																													" &vbCrlf
	sql	= sql & "And	 contentsNum	= '"& target &"'																					" &vbCrlf
	sql	= sql & "Order by step asc																														" &vbCrlf	
	'response.write Replace(sql,vbCrlf,"<br/>") : response.end
	Call getDicListData(sql, data)
